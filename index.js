...
function _createClient(name, options, callback) {
  ...
  client.on("error", function(err) {
    log.error({ err: err }, "kth-node-redis: Redis client error");
    callback(err);
  });

  client.on("warning", function(err) {
    log.warn({ err: err }, "kth-node-redis: Redis client warning");
  });
  ...
  client.on("ready", function() {
    ...
    log.debug(
      `kth-node-redis: Redis server version: ${safeGet(
  ...
  client.on("end", function() {
    ...
    if (!isReady) {
      callback(new Error("kth-node-redis: Failed to connect to Redis"));
    }
  });
}
...
module.exports = function(name, options) {
  ...
  if (client) {
    logger.debug(`kth-node-redis: Redis using client: ${name}`);
...
module.exports.connectClient = function(clientName) {
  ...
  if (!client) throw new Error("kth-node-redis: No such client: " + clientName);
