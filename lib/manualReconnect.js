...
function forgetDisconnectedClient(name) {
  ...
  redisClient.on('error', () => {
    logger.fatal(
      'kth-node-redis: Avoiding unhandled exception - Earlier disconnected Redis client unexpectedly still receives error messages - Are you caching the Redis client in your application?',
      { name }
    )
  })
