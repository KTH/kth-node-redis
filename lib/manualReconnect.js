...
function markClientAsDisconnected(name) {
  ...
  Global.clientData[name].disconnectedTimestamp = now
...
function checkErrorAndReconnectIfNeeded({ name, error, createNewClient }) {
  ...
  const now = new Date().getTime()
  const timeSinceDisconnect = now - disconnectedTimestamp
...
function forgetDisconnectedClient(name) {
  ...
  logger.info('kth-node-redis: Cleaning up disconnected Redis client', { name })
  ...
  redisClient.on('error', () => {
    logger.fatal(
      'kth-node-redis: Avoiding unhandled exception - Disconnected Redis client unexpectedly still receives error messages',
      { name }
    )
  })
