...
function _getDefaultRedisOptions(name) {
  const defaults = {
    retry_strategy: options => {
      ...
      if (retryInfo.totalRetryTime > ONE_MINUTE) {
      ...
      const delay = retryInfo.attempt ? Math.min(retryInfo.attempt * 300, 60000) : 60000
...
function _createClient(name, options, callback) {
  ...
  eventHandlers.error = error => {
    if (error && !error.toString().match(/(retry aborted|AbortError|max number of clients reached|command aborted)/i)) {
      log.warn('kth-node-redis: Redis client error', { event: 'error', error })
      // Not sure why "callbackOnce(error)" was only inside this conditional
    }
    log.error('kth-node-redis: Redis client error terminated', { event: 'error', error })

    // Delete the current client on error, for example exhausted retry.
    // This works fine for the cortina implementation in 'kth-node-web-common', unsure how it works with other implementations
    callbackOnce(error)
