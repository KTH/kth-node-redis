...
function _getDefaultRedisOptions(name) {
  const defaults = {
    retry_strategy: options => {
      ...
      const delay = retryInfo.attempt ? Math.min(retryInfo.attempt * 300, 3000) : 3000
      logger.info(
        `kth-node-redis - retry_strategy() is requesting retry after ${delay} ms`,
        retryInfo
      )
      return delay
    },
...
module.exports._testInternals = {
  async resetRedisClients() {
    const activeClients = Object.keys(Global.clients)
    if (activeClients.length > 0) {
      const client = Global.clients[activeClients[0]]
      if (client._redisMock != null) {
        client._redisMock.removeAllListeners()
      }
    }
    Global.clients = {}
  },
